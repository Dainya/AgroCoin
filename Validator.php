...
<?php
      ...
      $containsUpper = preg_match_all('/[^a-z\d!@#$%^&*()_+{}?><~`.,-=]/', $Password,$uppercase_match); // check if contains upper case
          ...
          if(!$containsDigit){// if its not containsDigit then print a message saying so
            $errors[] ="<br><span style=color:red>- Password must contain atleast one number.</span>";
            $ctr++;
            //1^%&dDDD85
          }

          if(!$containsSpecial){// if its not containsSpecial then print a message saying so
            $errors[] = "<br><span style=color:red>- Password must contain at least one special character.</span>";
            $ctr++;
            
          }

          if(!$containsLower){ // if its not containsLower then print a message saying so
            $errors[] = "<br><span style=color:red>- Password must contain one lowercase</span>";
            $ctr++;
          ...
          if(count($uppercase_match[0])  <  3){// if the variable uppercase_match is less than 3, then print a message saying so
            $errors[] = "<br><span style=color:red>- Password must contain three uppercase</span>";
            $ctr++;
          
          }

          if(strpos($Password," ")){// if password containes space
            $errors[] = "<br><span style=color:red>- Password must not contain spaces</span>";
            $ctr++;
          
          }

          if(strlen($Password) < 8 || strlen($Password) > 16){ //checking length if its between 8 and 16, is not print appropriate message
            $errors[] = "<br><span style=color:red>- Password must be between 8-16 characters</span>";
                $ctr++;
...
<?php
    ...
    $containsDigit = preg_match( '/^[+]?([\d]{0,3})?[\(\.\-\s]?([\d]{3})[\)\.\-\s]*([\d]{3})[\.\-\s]?([\d]{4})$/', $telephone);

    	 if(!$containsDigit){// if its not containsDigit then print a message saying so
            $errors[] ="<br><span style=color:red>- Telephone number is invalid.</span>";
            $ctr++;
